...
Similar to the computation for the Varma and Simon (2006) paper, the Nearest Centroid classifier within the scikit-learn library was used. The shrink_threshold parameter was used as the paper's delta hyperparameter. Ten values of delta were evaluated, ranging from 0.01 to 1.0.
```{python}
import numpy as np
...
The above procedure was then repeated for 50 simulations to determine the distribution of error estimates using the 0.632+ methodology. The computation was timed to determine how long it took to run these simulations.
```{python}
...
Then the nested CV error distributions over all of the simulations were plotted to compare their results to the 0.632+ error estimation approach.
```{python}
import matplotlib.pyplot as plt
